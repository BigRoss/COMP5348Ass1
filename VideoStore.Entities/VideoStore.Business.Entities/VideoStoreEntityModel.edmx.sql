...
-- --------------------------------------------------
-- Date Created: 04/14/2016 16:42:54
-- Generated from EDMX file: C:\Users\ajs_f\Documents\Assignment\VideoStoreS12016TestedLabs\VideoStore.Entities\VideoStore.Business.Entities\VideoStoreEntityModel.edmx
-- --------------------------------------------------
...
-- Creating foreign key on [Like_Id] in table 'Recommendations'
ALTER TABLE [dbo].[Recommendations]
ADD CONSTRAINT [FK_RecommendationLike]
    FOREIGN KEY ([Like_Id])
    REFERENCES [dbo].[Likes]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_RecommendationLike'
CREATE INDEX [IX_FK_RecommendationLike]
ON [dbo].[Recommendations]
    ([Like_Id]);
GO

-- Creating foreign key on [Media_Id] in table 'Likes'
ALTER TABLE [dbo].[Likes]
ADD CONSTRAINT [FK_LikeMedia]
    FOREIGN KEY ([Media_Id])
    REFERENCES [dbo].[Media]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_LikeMedia'
CREATE INDEX [IX_FK_LikeMedia]
ON [dbo].[Likes]
    ([Media_Id]);
GO

-- Creating foreign key on [Media_Id] in table 'Recommendations'
ALTER TABLE [dbo].[Recommendations]
ADD CONSTRAINT [FK_MediaRecommendation]
    FOREIGN KEY ([Media_Id])
    REFERENCES [dbo].[Media]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_MediaRecommendation'
CREATE INDEX [IX_FK_MediaRecommendation]
ON [dbo].[Recommendations]
    ([Media_Id]);
GO
